...
static llvm::Value* getMemoryNumPages(EmitFunctionContext& functionContext, Uptr memoryIndex)
{
	...
	llvm::LoadInst* memoryNumPagesLoad = functionContext.loadFromUntypedPointer(
		functionContext.createInBoundsGEP(
											   ...
											   BoundsCheckOp boundsCheckOp)
{
	...
	else
	{
		...
		llvm::Value* endAddress
			= functionContext.createLoad(functionContext.moduleContext.iptrType,
														 ...
														 Uptr memoryIndex)
{
	llvm::Value* memoryBasePointer
		= createLoad(llvmContext.i8PtrType, memoryInfos[memoryIndex].basePointerVariable);
	llvm::Value* bytePointer = createInBoundsGEP(llvmContext.i8Type, memoryBasePointer, boundedAddress);
...
#define EMIT_LOAD_OP(destType, name, llvmMemoryType, numBytesLog2, conversionOp)                   \
	...
	{                                                                                              \
		...
		auto load = createLoad(llvmMemoryType, pointer);                                           \
						 ...
						 Uptr numBytesLog2)
{
	...
	llvm::LoadInst* load = functionContext.createLoad(llvmVectorType->getScalarType(), pointer);
...
#define EMIT_ATOMIC_RMW(                                                                           \
	...
	{                                                                                              \
		...
		trapIfMisalignedAtomic(boundedAddress, numBytesLog2);                                      \
		auto pointer = coerceAddressToPointer(boundedAddress, llvmMemoryType, imm.memoryIndex);    \
		auto atomicRMW = createAtomicRMW(llvm::AtomicRMWInst::BinOp::rmwOpId,                      \
										 pointer,                                                  \
										 value,                                                    \
										 U64(1 << numBytesLog2),                                                        \
										 llvm::AtomicOrdering::SequentiallyConsistent);            \
